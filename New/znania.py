...
# в языке Python есть оператор in который возвращает true если находит что то в чем то.

# Составной оператор not in  возвращает true если в колекции нет этого елемента.
...
#.extend() - этот метод добаляет в конец списка каждый элемент отдельно. лучше .append()
...
#  Работа со славарем Dickt
#Ключь уникальное значение, если генерируеться еще одно точно такое еще оно не сохраниться в новом ключе_значении а просто перезапишет существющий.
...
# Работа с терминалом.
# Cntrl + Ё открыть \ закрыть терминал в VcCode

# PowerShell
# создать фаил 
# ni -type file -name "имя и расширение файла."  
# del "путь к файлу"
# dir все что есть в директори так же работает ls


# GitBash 
# pwd - ткущая директория

# сd название папки - переход в папку с названием
# cd .. - возврат \ переход на уровень выше
# cd /  - переход в корневую дректорию
# cd ~ - переход в домашнюю директрию

# touch file.txt  - создание файла
# rm file.txt - удаление файла
# mv file.txt newFolder - перемещение
# code text.txt  - открыть файл

# ls  - простой список содержимого
